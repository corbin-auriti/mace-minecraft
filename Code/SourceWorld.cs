...
namespace Mace
{
    static class SourceWorld
    {
        ...
        static RegionChunkManager _cmSource;
        ...
        public static bool SetupClass(AnvilWorld worldDest)
        {
            _AllEntityPainting = new EntityPainting[0];
            _AllBuildings = new Building[0];
            _intHouseNumber = 0;
            AnvilWorld worldSource = AnvilWorld.Open(Path.Combine("Resources", "Mace"));
        ...
        public static Building SelectRandomBuilding(BuildingTypes btNeeded, int intDepth)
        {
            ...
            {
                intFrequencyWeights = City.farmBuildingFrequency;
            }
            else
            {
                intFrequencyWeights = City.buildingFrequency;
            }
                                          ...
                                          int x1dest, int z1dest, Building bldInsert, int y1dest, int intRotationFixed)
        {
            ...
            //int intRandomWoodColour = RNG.Next(5);
                            ...
                            bmDest.GetID(intBlockStart + x + x1dest, yDest, intBlockStart + z + z1dest) != BlockInfo.WoodPlank.ID))
                        {
                            ...
                            switch (intBlockID)
                            {
                                ...
                                case BlockType.IRON_ORE:
                                    ...
                                    break;
                                //case BlockType.WOOD:
                                    //bmDest.SetData(intBlockStart + x + x1dest, yDest, intBlockStart + z + z1dest, intRandomWoodColour);
                                    //break;
        ...
        private static TileEntityChest MakeHouseChest()
        {
            ...
            for (int intItems = RNG.Next(2, 5); intItems >= 0; intItems--)
            {
                tec.Items[intItems] = BlockHelper.MakeItem(RNG.RandomItem(
                    ...
                    BlockInfo.Torch.ID,                    
                    ItemInfo.Wheat.ID,
                    ItemInfo.WoodenAxe.ID,
                    ItemInfo.WoodenHoe.ID,
                    ItemInfo.WoodenShovel.ID,
                    ItemInfo.WoodenSword.ID), 1);
            }
            return tec;
        }
        private static TileEntityChest MakeTreasureChest()
        {
            ...
            for (int intItems = RNG.Next(5, 7); intItems >= 2; intItems--)
            {
                tec.Items[intItems] = BlockHelper.MakeItem(RNG.RandomItem(
                    ...
                    ItemInfo.Diamond.ID,
                    ItemInfo.MusicDisc13.ID,
                    ItemInfo.MusicDiscCat.ID,
