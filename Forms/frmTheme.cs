...
namespace Mace
{
    public partial class frmTheme : Form
    {
        ...
        private void frmTheme_Load(object sender, EventArgs e)
        {
            ...
            clbMoatTypes.CheckAsterixedItems();
        ...
        private void cmbBuildingFrequency_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!isLoading)
            {
                ...
                if (building.GetData("name").Length > 0)
                {
                    ...
                    building.SetData("frequency", cmbBuildingFrequency.Text.ToLower());
                    clbBuildings_SelectedIndexChanged(sender, e);
                }
            }
        }
        ...
        private void CreateTheme()
        {
            using (StreamWriter writer = new StreamWriter(Path.Combine("Resources", "Themes", txtThemeName.Text + ".xml")))
            {
                ...
                writer.WriteLine("\t\t<city_prefix_file>" + cmbCityStartingWord.Text.Replace(" ", "_") + ".txt</city_prefix_file>");
                writer.WriteLine("\t\t<city_suffix_file>" + cmbCityEndingWord.Text.Replace(" ", "_") + ".txt</city_suffix_file>");
                writer.WriteLine("\t\t<building_frequency>default</building_frequency>");
                writer.WriteLine("\t\t<farm_building_frequency>default</farm_building_frequency>");
                if (cmbCityStartingWord.Text.Contains("Desert"))
                {
                    writer.WriteLine("\t\t<biome>desert</biome>");
                }
                else
                {
                    writer.WriteLine("\t\t<biome>plains</biome>");
                }
                writer.WriteLine("\t</options>");
                #endregion options
        ...
        private static string TextToBlocks(string[] identifiers)
        {
            ...
            blocks.Add("sapling: birch", BlockInfo.Sapling.ID + "_" + (int)SaplingType.BIRCH);
            blocks.Add("sapling: oak", BlockInfo.Sapling.ID + "_" + (int)SaplingType.OAK);
            blocks.Add("sapling: spruce", BlockInfo.Sapling.ID + "_" + (int)SaplingType.SPRUCE);
            ...
            switch (section)
            {
                case "features":
                    switch (item)
                    {
                        ...
                        case "main streets":
                            return "Wide paths that split the city into four quarters.";
                        case "mineshaft":
