...
namespace Mace
{
    class Farms
    {
        ...
        private static void MakePond(BlockManager bm, int x, int xlen, int z, int zlen, bool booMini)
        {
            ...
            if (!booMini)
            {
                c = rand.Next(1, intArea.GetUpperBound(0) - 7);
                d = rand.Next(1, intArea.GetUpperBound(1) - 7);
                clen = rand.Next(3, 7);
                dlen = rand.Next(3, 7);
            }
            ...
            intArea = CheckAgainstNeighbours(intArea, 0, 0, 7, 0, rand.Next(3, 6), true);
            intArea = CheckAgainstNeighbours(intArea, 6, 0, 2, 2);
            intArea = CheckAgainstNeighbours(intArea, 14, 0, 3, 3);
            ...
            for (a = 1; a <= intArea.GetUpperBound(0) - 1; a++)
            {
                for (b = 1; b <= intArea.GetUpperBound(1) - 1; b++)
                {
                    if (intArea[a, b] == 1)
                    {
                        ...
                        bm.SetData(x + a, 64, z + b, rand.Next(1, 3));
                    }
                    else
                    {
        ...
        private static void MakeHill(BlockManager bm, int x, int xlen, int z, int zlen, bool booMini)
        {
            ...
            intArea = CheckAgainstNeighbours(intArea, 0, 0, 7, 0, rand.Next(7, 11), true);
            intArea = CheckAgainstNeighbours(intArea, 6, 0, 3, 2);
            intArea = CheckAgainstNeighbours(intArea, 14, 0, 3, 3);
                        ...
                        if (y == intArea[a, b])
                        {
                            ...
                            switch (rand.Next(0, 10))
                            {
                                ...
                                case 3:
                                    ...
                                    bm.SetData(x + a, y + 64, z + b, rand.Next(1, 3));
                                    break;
                            }
                        }
                        else
        ...
        private static int[,] CheckAgainstNeighbours(int[,] intArea, int intMin, int intRandMin, int intRandMax, int intSet, int intTimes = 1, bool booCheckDown = false)
        {
            ...
            for (int intTime = 1; intTime <= intTimes; intTime++)
            {
                ...
                for (int a = 1; a <= intArea.GetUpperBound(0) - 1; a++)
                {
                    for (int b = 1; b <= intArea.GetUpperBound(1) - 1; b++)
                    {
                        if (booCheckDown)
                        {
                            if (Neighbours(intOriginal, a, b) < intMin + rand.Next(intRandMin, intRandMax))
                            {
                                intArea[a, b] = intSet;
                            }
                        }
            ...
            for (int x = 0; x <= intMapSize; x++)
            {
                for (int z = 0; z <= intMapSize; z++)
                {
                        ...
                        x >= intMapSize - intFarmSize || z >= intMapSize - intFarmSize)
                    {
                        ...
                        if (booFree && rand.NextDouble() > 0.90)
                        {
                            switch (rand.Next(4))
                            {
                                case 0:
                                ...
                                case 3:
                                    ...
                                    bm.SetData(x, 64, z, rand.Next(1, 3));
                                    break;
                            }
                        }
                    }
