...
namespace Mace
{
    class Walls
    {
                                     ...
                                     string strCityEmblem, string strOutsideLights)
        {
            ...
            {
                BlockShapes.MakeHollowLayers(a, intMapSize - a, 58, 72, a, intMapSize - a, (int)BlockType.STONE, 0, -1);
                world.Save();
            }
            // outside and inside edges at the top
            BlockShapes.MakeHollowLayers(intFarmSize + 5, intMapSize - (intFarmSize + 5), 72, 73,
                                         intFarmSize + 5, intMapSize - (intFarmSize + 5), (int)BlockType.STONE, 0, -1);
            BlockShapes.MakeHollowLayers(intFarmSize + 11, intMapSize - (intFarmSize + 11), 72, 73,
                                         intFarmSize + 11, intMapSize - (intFarmSize + 11), (int)BlockType.STONE, 0, -1);
            // alternating blocks on top of the edges
            for (int a = intFarmSize + 6; a <= intMapSize - (intFarmSize + 6); a += 2)
            {
                BlockShapes.MakeBlock(a, 74, intFarmSize + 5, (int)BlockType.STONE, 2, 100, -1);
            }
            for (int a = intFarmSize + 10; a <= intMapSize - (intFarmSize + 10); a += 2)
            {
                BlockShapes.MakeBlock(a, 74, intFarmSize + 11, (int)BlockType.STONE, 2, 100, -1);
            }
            // ladder
            BlockHelper.MakeLadder((intMapSize / 2) - 5, 64, 72, intFarmSize + 11, 2, (int)BlockType.STONE);
        ...
        private static void MakeEmblem(int intFarmSize, int intMapSize, string strCityEmblem)
        {
            if (strCityEmblem.ToLower() != "none")
            {
                ...
                {
                    strEmblem = File.ReadAllLines("Resources\\Emblem " + strCityEmblem + ".txt");
                }
