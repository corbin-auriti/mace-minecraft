...
namespace Mace
{
    static class BlockHelper
    {
        ...
        {
            BlockShapes.MakeBlock(x, y, z, BlockType.TORCH, BlockDirection(x, y, z, intBlockAgainst));
            if (intMirror > 0)
            {
                MakeTorch(_intMapSize - x, y, z, intBlockAgainst, 0);
                MakeTorch(x, y, _intMapSize - z, intBlockAgainst, 0);
                MakeTorch(_intMapSize - x, y, _intMapSize - z, intBlockAgainst, 0);
                if (intMirror == 2)
                {
        ...
        {
            if (_bmDest.GetID(x, y, z - 1) == BlockType.RAILS &&
                _bmDest.GetID(x - 1, y, z) == BlockType.RAILS)
            {
                _bmDest.SetData(x, y, z, 8);
            }
            else if (_bmDest.GetID(x, y, z - 1) == BlockType.RAILS &&
                     _bmDest.GetID(x + 1, y, z) == BlockType.RAILS)
            {
            ...
            else if (_bmDest.GetID(x + 1, y, z) == BlockType.RAILS)
            {
