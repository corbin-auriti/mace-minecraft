...
namespace Mace
{
    ...
    {
        static int _intCitySeed;
        static int _intWorldSeed;
        static bool[] _booSignUsed;
        static List<string> _lstMessages = new List<string>();

        public static void SetupClass(int intCitySeedOriginal, int intWorldSeedOriginal)
        {
            _intCitySeed = intCitySeedOriginal;
            _intWorldSeed = intWorldSeedOriginal;
            _booSignUsed = new bool[14];
            _lstMessages.Clear();
        }
        ...
        public static string GenerateNoticeboardSign(string strOverwrite)
        {
            switch (strOverwrite.ToLower().Substring(0, 5))
            {
                ...
                case "[nb2]":
                    return String.Format("City seed:~{0}~World seed:~{1}", _intCitySeed, _intWorldSeed);
                default:
                    return RandomSign();
            }
        }        
        ...
        {            
            ...
            do
            {
                intRand = RandomHelper.Next(_booSignUsed.GetLength(0));
            ...
            do
            {
                switch (intRand)
                {
                    ...
                    case 3: strSignText = RandomHelper.RandomString("Mrs", "Miss") + " " +
                                          RandomHelper.RandomFileLine(Path.Combine("Resources", "CityAdj.txt")) +
                                            " has lost her " +
