...
namespace Mace
{
    class BlockShapes
    {
        ...
        public static void MakeSolidBoxWithData(int x1, int x2, int y1, int y2, int z1, int z2, int intBlock, int intMirror, int intData)
        {
            for (int x = x1; x <= x2; x++)
            {
                for (int y = y1; y <= y2; y++)
                {
                    for (int z = z1; z <= z2; z++)
                    {
                        ...
                        {
                            bm.SetID(intMapSize - x, y, z, intBlock);
                            bm.SetData(intMapSize - x, y, z, intData);
                            bm.SetID(x, y, intMapSize - z, intBlock);
                            bm.SetData(x, y, intMapSize - z, intData);
                            bm.SetID(intMapSize - x, y, intMapSize - z, intBlock);
                            bm.SetData(intMapSize - x, y, intMapSize - z, intData);
                            if (intMirror == 2)
                            {
        ...
        public static void MakeHollowBox(int x1, int x2, int y1, int y2, int z1, int z2, int intBlock, int intMirror, int intData)
        {
            for (int x = x1; x <= x2; x++)
            {
                for (int y = y1; y <= y2; y++)
        ...
        /// </summary>
        public static void MakeHollowLayers(int x1, int x2, int y1, int y2, int z1, int z2, int intBlock, int intMirror, int intData)
        {
            for (int x = x1; x <= x2; x++)
            {
                for (int z = z1; z <= z2; z++)
        ...
        // xtodo: mirroring, doors, chests
        //public static void RotateBlocks(int x1, int x2, int y1, int y2, int z1, int z2, int intRotate = -1)
        //{
        //    // must be square
        //    if (intRotate < 0 || intRotate > 3)
        //    {
        //        intRotate = RandomHelper.Next(4);
        //    }
        //    if (intRotate > 0 && x1 - x2 == z1 - z2)
        //    {
