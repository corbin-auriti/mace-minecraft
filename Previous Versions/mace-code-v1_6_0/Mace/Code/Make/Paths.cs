...
namespace Mace
{
    class Paths
    {
        ...
        static List<structDistrict> lstDistricts = new List<structDistrict>();
        static List<string> lstStreetsUsed = new List<string>();
        static List<int> lstAllBuildings = new List<int>();
                                            ...
                                            int intStartX, int intStartZ, int intSizeX, int intSizeZ)
        {
            ...
            {
                if (lstAllBuildings.Contains(bldCheck.intID) || lstBuildings.Contains(bldCheck.intID))
                {
                    return false;
                }
                else
                {
                    ...
                    for (int x = intStartX - 10; x < intStartX + intSizeX + 10; x++)
                    {
                        for (int z = intStartZ - 10; z < intStartZ + intSizeZ + 10; z++)
                        {
        ...
        private static void MakeStreetLight(BlockManager bm, int x1, int y1, int x2, int y2)
        {
            if (bm.GetID(x1, 64, y1) == (int)BlockType.AIR)
            {
                ...
                bm.SetID(x1, 68, y1, (int)BlockType.FENCE);
                bm.SetID(x2, 68, y2, (int)BlockType.FENCE);
                bm.SetID(x2, 67, y2, (int)BlockType.GLOWSTONE_BLOCK);
