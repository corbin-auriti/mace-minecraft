...
namespace Mace
{
    class BlockShapes
    {
        ...
        // this is like making a hollow box, but the top and bottom aren't filled in
        ...
        /// <summary>Turns a three-dimensional array into a string, suitable for printing to the console</summary>
        static string StringFromArray3D(string[, ,] strArray)
        {
            string strOutput = "";
            for (int x = 0; x <= strArray.GetUpperBound(0); x++)
            {
                for (int y = 0; y <= strArray.GetUpperBound(1); y++)
                {
                    for (int z = 0; z <= strArray.GetUpperBound(2); z++)
                    {
                        strOutput += strArray[x, y, z].PadLeft(2) + " ";
                    }
                    strOutput += "\n";
                }
                strOutput += "\n";
